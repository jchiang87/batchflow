...
# ---------------------------------------------------------------------------
# eo_pipe adapter tests
# ---------------------------------------------------------------------------

class TestEoPipeAdapter:
    """Tests for the eo_pipe legacy config adapter."""

    def _write_config(self, tmp_path, extra_run_type=None) -> Path:
        cfg = {
            "baseline": {
            ...
            "b_protocol": {
                ...
                "pipelines": [
                    ...
                    "bps_eoLinearity.yaml",
                ],
            },
            "ptc": {
                "env_vars": [],
                "pipelines": ["bps_cpPtc.yaml", "bps_eoLinearity.yaml"],
            },
        }
        if extra_run_type:
            cfg.update(extra_run_type)
        path = tmp_path / "eo_pipelines_config.yaml"
        import yaml
        path.write_text(yaml.dump(cfg))
        return path

    def test_load_flat_run_type(self, tmp_path):
        from eo_pipe_adapter import load_eo_workflow
        cfg = self._write_config(tmp_path)
        g = load_eo_workflow(cfg, "ptc")
        assert g.workflow_id == "ptc"
        assert len(g.nodes) == 2
        # Flat (no deps) → all should be roots
        ready = list(g.ready_nodes())
        assert len(ready) == 2

    def test_load_with_explicit_dependencies(self, tmp_path):
        from eo_pipe_adapter import load_eo_workflow
        extra = {
            "dag_protocol": {
                "pipelines": [
                ...
                "dependencies": {
                    "bps_cpPtc.yaml": ["bps_eoReadNoise.yaml"],
                    "bps_eoLinearity.yaml": ["bps_cpPtc.yaml"],
                },
            }
        }
        cfg = self._write_config(tmp_path, extra_run_type=extra)
        g = load_eo_workflow(cfg, "dag_protocol")
        # Only read_noise is a root
        ready = list(g.ready_nodes())
        assert len(ready) == 1
        assert ready[0].node_id == "eo_read_noise"
        # ptc depends on read_noise, linearity depends on ptc
        assert g.node("cp_ptc").depends_on == ["eo_read_noise"]
        assert g.node("eo_linearity").depends_on == ["cp_ptc"]

    def test_unknown_run_type_raises(self, tmp_path):
        from eo_pipe_adapter import load_eo_workflow
        cfg = self._write_config(tmp_path)
        with pytest.raises(KeyError, match="no_such_type"):
            load_eo_workflow(cfg, "no_such_type")

    def test_yaml_to_node_id_conversion(self):
        from eo_pipe_adapter.adapter import _yaml_to_node_id
        assert _yaml_to_node_id("bps_eoReadNoise.yaml")      == "eo_read_noise"
        assert _yaml_to_node_id("bps_cpPtc.yaml")            == "cp_ptc"
        assert _yaml_to_node_id("bps_eoBiasStability.yaml")  == "eo_bias_stability"
        assert _yaml_to_node_id("bps_eoDarkDefects.yaml")    == "eo_dark_defects"
        assert _yaml_to_node_id("bps_eoLinearity.yaml")      == "eo_linearity"

    def test_check_env_vars_missing(self, tmp_path, monkeypatch):
        from eo_pipe_adapter import check_env_vars
        cfg = self._write_config(tmp_path)
        monkeypatch.delenv("BUTLER_CONFIG", raising=False)
        monkeypatch.delenv("INSTRUMENT_NAME", raising=False)
        missing = check_env_vars(cfg, "b_protocol")
        assert "BUTLER_CONFIG" in missing
        assert "INSTRUMENT_NAME" in missing

    def test_check_env_vars_all_set(self, tmp_path, monkeypatch):
        from eo_pipe_adapter import check_env_vars
        cfg = self._write_config(tmp_path)
        monkeypatch.setenv("BUTLER_CONFIG", "/repo")
        monkeypatch.setenv("INSTRUMENT_NAME", "LSSTCam")
        missing = check_env_vars(cfg, "b_protocol")
        assert missing == []

    def test_register_lsst_patterns(self):
        from eo_pipe_adapter import register_lsst_patterns
        clf = ErrorClassifier()
        register_lsst_patterns(clf)
        result = clf.classify(["DatasetNotFoundError: dataset missing"])
        assert result.verdict == "fatal"
        result2 = clf.classify(["Butler datastore connection timeout"])
        assert result2.verdict == "transient"

    def test_metadata_carries_run_type(self, tmp_path):
        from eo_pipe_adapter import load_eo_workflow
        cfg = self._write_config(tmp_path)
        g = load_eo_workflow(cfg, "ptc")
        for node in g.nodes:
            assert node.metadata["run_type"] == "ptc"
